...
async fn get_file_metadata_text_file() {
    ...
    let svc = make_service(dir.path());
    let meta = svc
        .get_file_metadata(file.to_str().unwrap())
        .await
        .unwrap();
...
async fn get_file_metadata_image() {
    ...
    let svc = make_service(dir.path());
    let meta = svc
        .get_file_metadata(png.to_str().unwrap())
        .await
        .unwrap();
...
async fn get_file_metadata_directory() {
    ...
    let svc = make_service(dir.path());
    let meta = svc
        .get_file_metadata(sub.to_str().unwrap())
        .await
        .unwrap();
...
async fn get_file_metadata_nonexistent() {
    ...
    let svc = make_service(dir.path());
    let result = svc
        .get_file_metadata(fake.to_str().unwrap())
        .await;
...
async fn get_file_metadata_outside_sandbox() {
    ...
    let svc = make_service(sandbox.path());
    let result = svc
        .get_file_metadata(secret.to_str().unwrap())
        .await;
