//! Cross-platform cache directory resolution for the bundled node runtime.

use std::path::{Path, PathBuf};
use std::sync::OnceLock;

/// Override for [`runtime_root`], set by [`init`] from the backend
/// startup path so cached node binaries land under `AppConfig.data_dir`
/// instead of the OS-default cache location.
///
/// Lifecycle: written once by `aionui-app`'s `main()` before
/// [`crate::enhance_process_path`] / [`crate::resolve_node`] run, read
